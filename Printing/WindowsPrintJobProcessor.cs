...
namespace CloudPrint.Printing
{
    public class WindowsPrintJobProcessor : IPrintJobProcessor
    {
        ...
        {
            if (DateTime.Now.AddDays(-Config.MaxJobAgeInDays) > job.CreateTime)
            {
        ...
        private IEnumerable<string> EnumerateLocalPrinterNames()
        {
            ...
            try
            {
                while (!EnumPrinters(PRINTER_ENUM.LOCAL | PRINTER_ENUM.SHARED | PRINTER_ENUM.NETWORK | PRINTER_ENUM.CONNECTIONS | PRINTER_ENUM.DEFAULT | PRINTER_ENUM.NAME, null, 4, buf, bufsize, out cbneeded, out nents))
                {
                    if (cbneeded == bufsize)
                    {
